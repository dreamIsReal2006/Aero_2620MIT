...
Feed reads use the browser Supabase client from `lib/supabaseClient.js`. Configure Supabase RLS policies to allow the intended anon `select` access for `posts`, `users`, and `follows`; missing configuration, denied reads, and empty results render the normal empty states instead of interrupting the page.
