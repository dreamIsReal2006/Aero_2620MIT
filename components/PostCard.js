...
export default function PostCard({ post, onAction }) {
  ...
  return <Glass id={`post-${post.id}`} data-post-id={post.id} className="aero-pop mb-5 rounded-[20px] p-5 transition-transform duration-200 hover:-translate-y-0.5">
    ...
    {media.length > 0 && <div className={`post-media-container mb-3 ${media.length > 1 ? 'post-media-carousel' : ''}`}>{media.map((item, index) => { const url = mediaUrl(item); const video = isVideo(item); return video ? <video key={`${item}-${index}`} src={url} className="post-media-item" preload="metadata" controls playsInline crossOrigin="anonymous" /> : <img key={`${item}-${index}`} src={url} alt="Post media" className="post-media-item" loading="lazy" onClick={(event) => { event.preventDefault(); event.stopPropagation(); setViewerIndex(index); }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); event.stopPropagation(); setViewerIndex(index); } }} tabIndex={0} role="button" aria-label={`Open media ${index + 1} of ${media.length}`} />; })}</div>}
